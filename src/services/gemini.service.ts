...
class GeminiService {
  ...
  private getCandidateKeys(): string[] {
    ...
    if (process.env.GEMINI_API_KEY) {
      ...
      if (envKey && envKey !== 'mock-key' && envKey !== 'sk-ant-api03-mock-key') {
        keys.push(envKey);
      }
    }
    if (!keys.includes(userDefaultKey)) {
      keys.push(userDefaultKey);
    }
    return keys;
  }
...
- الوقت الحالي للنظام هو: ${new Date().toISOString()}. استخدم هذا المرجع لتحديد الأوقات النسبية (مثل اليوم، غداً، إلخ).` + customInstructions;
    ...
    for (const apiKey of candidateKeys) {
      ...
      for (const targetModelName of candidateModels) {
        try {
          console.log(`[Gemini AI] المحاولة باستخدام الموديل: ${targetModelName} والرمز المفتاحي...`);
          ...
          lastError = error;
          console.warn(`[Gemini AI] الموديل ${targetModelName} فشل (${error?.message || error}). تجربة الموديل/المفتاح التالي...`);
          continue;
        }
      }
    }
...
تنبيه: يجب دائماً استدعاء أداة "update_restaurant_instructions" عند حدوث أي تعديل في القواعد لضمان حفظها في قاعدة البيانات.`;
    ...
    for (const apiKey of candidateKeys) {
      ...
      for (const targetModelName of candidateModels) {
        try {
          ...
          lastError = err;
          console.warn(`[Gemini Admin Config] الموديل ${targetModelName} فشل (${err?.message || err}). تجربة المفتاح/الموديل التالي...`);
          continue;
        }
      }
    }
  ...
  private generateMockResponse(message: string, restaurantName: string): string {
    ...
    if (cleanMsg.includes('منيو') || cleanMsg.includes('قائمة') || cleanMsg.includes('أكل') || cleanMsg.includes('طعام')) {
      return `أهلاً بك في مطعم ${restaurantName}!
قائمة الطعام المتاحة لدينا اليوم (الأسعار بالجنيه المصري):
1. وجبة شاورما دجاج سوبر - 85.00 ج.م
2. بيتزا مارغريتا عائلية - 145.00 ج.م
3. عصير برتقال طازج - 40.00 ج.م

هل تحب تسجل طلب بأي من الأصناف دي؟`;
    }

    if (cleanMsg.includes('حجز') || cleanMsg.includes('طاولة')) {
      return `أهلاً بك! لحجز طاولة في مطعم ${restaurantName}، يرجى تزويدي باليوم والوقت المطلوب وعدد الأفراد. مثال: (اليوم الساعة 8 مساءً لـ 4 أفراد).`;
    }

    if (cleanMsg.includes('طلب') || cleanMsg.includes('أريد أن أطلب')) {
      return `أهلاً بك! يرجى تحديد الوجبة والكمية المطلوبة من المنيو وسأقوم بتسجيل الطلب فوراً وحساب الإجمالي بالجنيه المصري (ج.م).`;
    }

    return `أهلاً بك في مطعم ${restaurantName}! أنا المساعد الذكي المطور بنظام Rivix System.
أقدر أساعدك فوراً في استعراض المنيو والأسعار بالجنيه المصري، حجز طاولة، أو تسجيل طلب طعام. كيف يمكنني خدمتك اليوم؟`;
  }
}
